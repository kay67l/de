...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (type === 'event') {
      const { wing, title, description, photo_urls, video_url } = req.body || {};
      if (!WINGS.includes(wing)) return res.status(400).json({ error: 'Invalid wing.' });
      if (!title?.trim()) return res.status(400).json({ error: 'Title is required.' });
      if (video_url && !/^https?:\/\//i.test(video_url.trim())) {
        return res.status(400).json({ error: 'Video link must be a full URL.' });
      }
  ...
  if (req.method === 'PATCH') {
    ...
    if (type === 'event') {
      const { id, title, description, photo_urls, video_url } = req.body || {};
